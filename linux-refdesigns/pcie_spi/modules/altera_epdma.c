...
void block_ioctl_setup(struct dma_device *dmadev)
{
   unsigned int spi_rx_data = 0;
	...
	case SYS_RX_IOCTL:
		...
		break;
//	case SPI_TX_IOCTL:
//		csr_writel(dmadev->pio_base, 0xf, 0);
//		break;
//	case SPI_RX_IOCTL:
//		spi_rx_data = csr_readl(dmadev->pio_base, 0);
//		break;
	}
}
		       ...
		       const struct pci_device_id *pci_id)
{
    ...
    dmadev->pio_base = devm_ioremap(&pdev->dev, RP_PIO_SBASE, 0x1f);   // add ted
	if (!dmadev->pio_base) {
		dev_err(&pdev->dev, "failed to mapping rootport PIO\n");
		return -ENODEV;
	}
...
//	err = pcim_iomap_regions(pdev, BIT(EP_OCM_BAR_NR) | BIT(EP_CSR_BAR_NR), DRIVER_NAME);

	err = pcim_iomap_regions(pdev, BIT(EP_OCM_BAR_NR) | BIT(EP_CSR_BAR_NR) | BIT(EP_OCM2_BAR_NR) , DRIVER_NAME);
	if (err) {
	...
	dmadev->spi_base = pcim_iomap_table(pdev)[EP_OCM2_BAR_NR];
	if (!dmadev->spi_base) {
		dev_err(&pdev->dev, "failed to map EP OCM2 region\n");
		return err;
	}
    ...
    //add ted
/*
	unsigned int pcie_read_data = 0; 
//	csr_writel(spi_base, 0x1234, 0);
	csr_writel(dmadev->pio_base,0x0f,0);
	csr_writel(dmadev->pio_base,0x00,0);
	csr_writel(spi_base, 0x4321, 4);
	//csr_writel(spi_base, 0x1234, 8);
    pcie_read_data = csr_readl(spi_base, 32);
  //	while(pcie_read_data!=0x4321);
	csr_writel(dmadev->pio_base,0x0f,0);
	csr_writel(dmadev->pio_base,0x00,0);
//	csr_writel(spi_base, 0xdeadbeef, 4); 
*/
